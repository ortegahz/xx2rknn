...
if __name__ == '__main__':
    ...
    print('--> config model')
    # ([0 255] - 0.0) / 255.0 --> [0 1]
    # rknn.config(channel_mean_value='0. 0. 0. 255.', reorder_channel='0 1 2', target_platform=['rk3399pro'])
    rknn.config(channel_mean_value='0. 0. 0. 255.', reorder_channel='0 1 2', target_platform=['rv1126'])
    print('done')

    # Load tensorflow model
    print('--> Loading model')
    ret = rknn.load_caffe(model='/media/manu/intel/workspace/onnx2caffe/model/bestc.prototxt',
                          proto='caffe',
                          blobs='/media/manu/intel/workspace/onnx2caffe/model/bestc.caffemodel')
    if ret != 0:
